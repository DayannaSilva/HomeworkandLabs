...
//    Date1 date = new Date1(0,0,0);
    ...
    public static boolean checkStreet(String streetFirst, String streetSecond, String city, String country){
        if(onlyString(streetFirst) && onlyString(streetSecond) && onlyString(city) && onlyString(country))
            return true;
        else
            return false;
    }
    public static boolean valuePay(int valueI, int valueP){
        if(valueI<=valueP)
            return false;
        else
            if(valueI<=0 || valueP<0)
                return false;
            else
                return true;
    }
